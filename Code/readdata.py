...
for index, name in enumerate(classes): 
    ...
    for img_name in os.listdir(class_path):  
        ...
        img_re = tf.image.resize(img_tensor, [180, 180])
        img_re = img_re/255.0
        img_tmp.append(img_re)
        lable_tmp.append(index)
        print(type(img_tmp))
    image_label_ds = tf.data.Dataset.zip((img_tmp, lable_tmp))
    img_final.append(image_label_ds)
    print(len(img_final))
...
#             img_path = class_path + img_name  
#             img = Image.open(img_path)  
#             img = img.resize((160, 160))    #设置需要转换的图片大小
